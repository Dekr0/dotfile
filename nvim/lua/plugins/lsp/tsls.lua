return {
    ...
    root_dir = function(bufnr, on_dir)
        ...
        -- Select then from the project root, which is identified by the presence 
        -- of a package manager lock file.
        local root_markers = {
            'package-lock.json',
            'yarn.lock',
            'pnpm-lock.yaml',
    ...
    on_attach = function(client, bufnr)
        ...
        -- These only appear in `vim.lsp.buf.code_action()` if specified in 
        -- `context.only`.
